...
main(List<String> args) async {
  ...
  ///So what's going to happen if you won't use `asBroadcastStream()` (in line number 21) is
  ///the second listener will get triggered frist and then it will throw an error, but how?
  ///Our stream is a `single subscription stream` and 
  ///the first listener gets applied after the second listener get applied
  ///because it(first listener) has to perform transform operation(line 31,34) 
  ///on the data with two diffrent objects,
  ///therefore with Single Subscription Stream, we can listen to a stream only Once.
  ///However, with [BroadcastStream] we don't get gurantee of the order of the data.
