...
///If we do not go out of the way to spawn other threads with the use of [Isolates]
///then our code is fully single-threaded.
...
main(List<String> args) async {
  ...
  ///We are getting the value of the future from
  ///`loadStringWithAsyncAndAwait(str)` with the help of async keyword.
  ...
  ///Note that we have used `await` above,
  ///it will give use the permanence in the order and
  ///your code will wait rather than immediately
  ...
  ///for instance: If you won't call the above function and
  ///simply execute the below function,
  ///the statements under this below function will get exectued first
  ///because your `loadStringWithAsyncAndAwait`
...
/// So it's basically like creating a promise in the code that says 
/// "Okay, well eventually we will get a String(data)"
/// 
/// The way it works under the hood:
/// `loadString()` will make a future and return it to main function 
/// where it will sit there uncompleted and 
/// later when the value is available from this main fun, 
/// it will be pushed to that future object and that future obj will be completed.
...
/// so instead of creating a `Future` value, 
/// (like we did in the previous function) we just simply
/// return a String value and it will automatically put it in a different future object 
/// coz we are using [async] keyword.
/// 
/// TODO Imp. Point
/// So even if we are using a Future object inside this method, 
/// it is done only to delay the function.
/// the statement -> 
///   'return "String: $str";' will automatically return a `Future` with a String value in it.
